...
class FM_FTRL_machine(object):
    ...
    def __init__(self, fm_dim, fm_initDev, L1, L2, L1_fm, L2_fm, D, alpha, beta, alpha_fm=.1, beta_fm=1.0, dropoutRate=1.0):
        ...
        # learning rate parameter alpha for factorization machine
        self.alpha_fm = alpha_fm
        # learning rate parameter beta for factorization machine
        self.beta_fm = beta_fm
        # L1 regularizer for factorization machine weights. Only use L1 after
        # one epoch of training, because small initializations are needed for
        # gradient.
        self.L1_fm = L1_fm
        # L2 regularizer for factorization machine weights.
        self.L2_fm = L2_fm
        self.fm_dim = fm_dim            # dimension of factorization.
        # standard deviation for random intitialization of factorization
        # weights.
        self.fm_initDev = fm_initDev
        # dropout rate (which is actually the inclusion rate), i.e. dropoutRate
        # = .8 indicates a probability of .2 of dropping out a feature.
    ...
    def predict_raw(self, x):
        ...
        for i in x:
            ...
            for k in range(self.fm_dim):
                sign = - \
                    1. if z_fm[i][
                        k] < 0. else 1.   # get the sign of z_fm[i][k]

                if sign * z_fm[i][k] <= L1_fm:
                    w_fm[i][k] = 0.
                else:
                    w_fm[i][k] = (sign * L1_fm - z_fm[i][k]) / \
                        ((beta_fm + sqrt(n_fm[i][k])) / alpha_fm + L2_fm)
    ...
    def update(self, x, p, y):
        ...
        for i in x:
            for k in range(self.fm_dim):
                g_fm = g * fm_sum[i][k]
                sigma = (sqrt(n_fm[i][k] + g_fm * g_fm) -
                         sqrt(n_fm[i][k])) / alpha_fm
...
def data(filePath, hashSize, hashSalt):
    ...
    for t, row in enumerate(DictReader(open(filePath))):
        ...
        for key in row:
            ...
            # 1 is added to hash index because I want 0 to indicate the bias
            # term.
            index = abs(hash(hashSalt + key + '_' + value)) % hashSize + 1
            x.append(index)
